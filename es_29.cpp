...
bool modifica(prodotto p, prodotto elenco[], int dim)
{
	int pos = ricerca(p, elenco, dim);
	
	if(pos==-1){
		return false;
	}
	
	cout<<"inserisci il nome da sostituire";
	cin>>elenco[pos].nome;
	
	cout<<"inserisci la categoria da sostituire";
	cin>>elenco[pos].categoria;
	
	cout<<"inserisci il prezzo da sostituire";
	cin>>elenco[pos].prezzo;
	
	return true;
}

void inserimento(prodotto p, prdotto elenco[], int dim )
{
	int prima=
	for(int i=0; i<dim; i++)
	{
		if(elenco[i].nome == elenco[i+1].nome )
		{
			cout<<"la parola e' uguale";
		}
	}
}
